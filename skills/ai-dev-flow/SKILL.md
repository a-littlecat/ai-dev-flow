...
Tracked / Controlled 默认有 2 轮 `AutoRepair`；第 3 轮只能由 `CORE.md` 的 progress gate 授予。3 是自主循环上限，不是 AI 永久禁修：`Stop` 后用户可明确授权一次有界 `EscalatedRepair`，失败即回到 `Stop`。预算绑定 repair chain；换 TASK 或模型不重置，不可逆外部动作不得自动重试。只读 gate 只给机械资格，最终 Allowed 由持有真实对话/harness/项目证据的 Orchestrator 提升。
