...
`runtime_session_bridge` 是运行时会话事实接入能力；只有存在真实可执行入口且五个生命周期 Hook 齐全时才能声明 `type=command`。`formal_skill_sync_method` 是另行授权后的正式 Skill 同步说明。两者不得混用，前者永远不授予后者的外部写入权限。
