...
Batch、Wave、Loop 都不是任务状态。它们只表示任务的执行 / 审查 / 编排组织方式，不能替代 Draft、Ready、In Progress、Review、Accepted 或 Closed。

Intake 也不是任务状态。Intake 是需求进入 `plan_task` 或 `create_task` 前的记录层，不直接进入任务状态机。
...
| 已验收 | Accepted | 按用户动作等级完成必要验收或决策。用户或指定验收人确认可接受。 |
| 已关闭 | Closed | 任务记录、验收、后续建议均已收口，且关闭动作已由用户确认或项目规则授权。 |
...
- 不得因为 Loop 完成，就自动把其中所有任务标记为 Accepted 或 Closed。
- Loop State 应记录到 `docs/loops/LOOP_STATE.md` 或项目约定位置，不要混入任务状态字段。
