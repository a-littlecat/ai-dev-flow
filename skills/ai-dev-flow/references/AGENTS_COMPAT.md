...
- 用户反馈 bug、验收失败、实机失败或“不符合预期”时，不得直接猜测修改。
- 先定义 RED 失败信号、GREEN 通过信号和 SIGNAL 证据来源。
- agent 无法亲自复现时，应提供用户实机 HITL 步骤和回传证据模板。
- 根因不清时先进入 Bug 诊断（bug_diagnosis）。
- 可测试代码优先使用测试先行（tdd_task）。
- 临时诊断日志必须有唯一前缀，修复后必须清理。
- 没有测试信号或证据时，任务应建议阻塞（Blocked）或等待补充，不得盲修。
- 跨会话继续任务时，应生成会话交接（session_handoff）摘要。
