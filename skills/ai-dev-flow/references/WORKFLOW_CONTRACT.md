...
1. 文件出现精确二级标题 `## Workflow Contract` 时，必须进入 v0.7 Reader。
2. v0.7 文件缺少 `schema_version`、存在多个 Contract 块或 grammar 损坏时，输出 `E_PARSE`；不得降级为 Legacy Reader。
3. 文件完全不含该精确标题时，进入 Legacy Reader；但 canonical ``- `schema_version`:`` 键值行出现在块外时仍为 `E_PARSE`，不能用标题拼写错误逃逸严格校验。
4. 大小写或空白不同的近似标题不是 Contract 标题，也不是 legacy 别名；不得模糊匹配。
...
| `review_status` | 未审查 / 审查中 / 通过 / 已通过 / 复审通过 / 需要修改 / 不建议合并 | Pending / In Review / Passed / Passed / Passed / Needs Fix / Do Not Merge |
...
- `Review` 单元格只取 ` / ` 或 `；` 前的首段，并按第 10.3 节 Review 别名映射；额外摘要只保留 provenance。
...
|---|---|---|---|
| `E_PARSE` | error | 003 | Markdown/Contract 结构、未知 key、重复 key 或编码无法解析 |
...
Schema 不校验注入了 `Not Recorded` 的 Normalized View，也不覆盖 Markdown grammar、provenance、legacy 冲突、正文引用、Git 历史、看板、真实用户身份或外部事实。Reader/Validator 失败结果不要求伪装成一个 Schema-valid Contract。Schema 可接受显式安全 token `None`，但 Writer 仍应按第 3.1 节省略未触发的条件字段。
