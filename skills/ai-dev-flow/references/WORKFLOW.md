...
- 验证分两级执行（详见 `SKILL.md` 执行与验证节）：
  - 迭代验证：patch 期间优先运行最贴近风险的测试，再运行受影响的合理范围回归；相互独立的验证项并行运行，不在每轮 patch 重复完整套件。
  - 闸门验证：请求独立 Review、UA、merge 或 delivery 前，运行项目定义的完整验证套件一次，作为 Review / UA 的输入证据。
- 真实环境、真实账号、真实数据或外部宿主所需证据不能由普通单元测试替代；此类证据按项目定义在闸门验证或 UA 中取得。
