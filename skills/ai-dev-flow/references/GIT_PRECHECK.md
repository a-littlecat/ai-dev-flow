...
- 判断是否有未提交改动。
- 判断是否存在不应提交的文件。
- 如果是 Batch，判断批次内任务是否共享同一清晰 base commit。
- 如果是 Wave，判断每个任务是否有独立 base commit、执行位置和清晰工作区。
...
- Batch 仍需记录 base commit，并确认批次内 A/B 小任务之间没有冲突文件。
- Wave 必须为每个任务记录 base commit、分支或 Worktree、预计修改文件和影响模块。
