...
这些指南是 Markdown-first 模板，不表示自动脚本、自动同步、自动多 agent 调度、自动实机控制、自动合并或自动发布。
...
当前仓库内部版本身份为 `0.6.0`，状态为 **Release ready（尚未发布）**。计划 tag 为 `v0.6.0`；只有独立 Review 无 P0/P1、版本一致性验证通过且用户明确完成 UA7 授权后才允许创建。tag、push、GitHub Release、merge 和本机 Skill 同步均不在版本号更新的默认授权内。
...
- Engineer 不自我批准。
- 所有危险操作必须用户确认。
- 反馈闭环指南不代表自动实机控制、自动脚本、自动同步、自动合并或自动发布。
