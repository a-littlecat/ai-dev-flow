...
- `HARNESS_COMPAT.md`
- `BUG_DIAGNOSIS_GUIDE.md`
- `TDD_GUIDE.md`
- `REQUIREMENT_GRILLING_GUIDE.md`
- `PROJECT_CONTEXT_GUIDE.md`
- `HANDOFF_GUIDE.md`
- `ARCHITECTURE_REVIEW_GUIDE.md`
- `REAL_ENV_SIGNAL_GUIDE.md`
...
ai-dev-flow 不只管理任务状态，也帮助 agent 把用户反馈转成可复测信号：先记录 RED 失败信号、GREEN 通过信号和 SIGNAL 证据来源，再决定进入 Bug 诊断（`bug_diagnosis`）、修复任务（`repair_task`）、审查-修复循环（`review_repair_loop`）、创建新 TASK 或阻塞（Blocked）。

新增指南包括：

- Bug 诊断（`bug_diagnosis`）：根因不清时先建立证据和可证伪假设。
- 测试先行（`tdd_task`）：用 RED / GREEN 约束行为级测试或替代验证。
- 需求拷问（`requirement_grilling`）：需求模糊或高风险时只问阻塞问题。
- 项目语境（`project_context`）：沉淀长期术语、验证经验和常见误解。
- 会话交接（`session_handoff`）：跨会话保留下一步、应读文件和不要重复尝试的路线。
- 架构巡检（`architecture_review`）：只读发现反复返工、无测试缝隙或边界混乱。
- 实机测试信号复现（`real_env_signal`）：把用户真实环境失败转成 RED / GREEN / SIGNAL 和 HITL 回传证据。

这些指南是 Markdown-first 模板，不表示自动状态写入、自动同步、自动多 agent 调度、自动实机控制、自动合并或自动发布；v0.7.0 新增的脚本仅执行只读检查。
...
v0.7.0 在 v0.6.0 的 Markdown-first 工作流之上增加确定性的 Contract 与只读检查能力：

- `adf/v0.7.0` Workflow Contract 统一 lifecycle、Review、UA 与 delivery 语义。
- Reader 同时支持 legacy TASK 和显式 opt-in 的 v0.7 TASK，不自动迁移旧任务。
- `workflow_lint` 只读输出稳定的 Human/JSON diagnostics 和 provenance。
- 符合条件的新建 A/B 任务使用 Compact v0.7；复杂路径继续使用 Full/Legacy。
- TASK_BOARD 仅作为 TASK 的只读投影进行 drift 检查，不提供 `--fix` 或反向写回。

当前 Skill 分发包版本为 `0.7.0`，Contract 接口版本独立为 `adf/v0.7.0`。lint 通过只代表结构和当前可确定规则通过，不代表 Review、用户验收、merge、release 或任务关闭已经完成。

当前状态为 **Release ready（尚未发布）**。计划 tag 为 `v0.7.0`；只有独立 Review 无 P0/P1、版本一致性验证通过且用户重新完成 UA7 授权后才允许创建。tag、push、GitHub Release 和 merge 均不在版本号更新或本机 Skill 同步的默认授权内。

## v0.6.0：Intake、Loop、Roles、Memory
...
- Bug Diagnosis：证据优先的 bug 诊断流程。
- TDD：行为优先的 RED / GREEN 开发记录。
- Requirement Grilling：高风险或模糊需求的阻塞问题清单。
- Project Context：长期项目语境和验证经验沉淀。
- Session Handoff：跨会话继续任务的交接摘要。
- Architecture Review：只读架构阻力和测试缝隙巡检。
- Real Environment Signal：用户验收失败的 RED / GREEN / SIGNAL 处理。
...
- Engineer 不自我批准。
- 所有危险操作必须用户确认。
- 反馈闭环指南不代表自动实机控制、自动状态写入、自动同步、自动合并或自动发布。
...
只读 subagents 默认适合审查、验证、状态分拣、文档检查、日志整理和测试结果整理。写代码 subagents 可以受控使用，但必须声明任务编号、角色、模式、允许修改范围、禁止修改范围和验证方式，最终仍需 Reviewer 审查。

代码并行仍必须遵守 Parallel Wave：多个写代码 subagents 并行默认需要用户确认，代码任务默认使用独立分支或 Worktree，并逐任务审查。subagents 不得自动 merge、push、release 或 delete。
...
- 并行执行不是默认行为，必须用户确认。
- ai-dev-flow 不禁止 harness 或模型自动使用 subagents，但写代码 subagents 必须受控，并且不得绕过 TASK 边界、diff 记录、验证、审查和用户确认。
...
6. 主控 agent 先判断格式：新建 A/B、`overlays=none`、非 Batch/Wave/`real_env_signal` 使用 `references/TASK_TEMPLATE_COMPACT.md`；已确定为 C/D、Batch、Wave、`real_env_signal` 或 existing legacy 时使用 Full/Legacy `references/TASK_TEMPLATE.md`；条件未知时停止、记录“待确认”，不得选择任何模板。
...
v0.7.0 已提供标准库实现的只读 Reader、lint 和 TASK_BOARD drift 检查；仍不提供自动状态写入、自动 merge、自动发布或外部双向同步。
