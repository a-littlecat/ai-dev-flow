...
export class DetailPanel {
  ...
  private renderDetail(body: HTMLElement, detail: TaskDetail, state: AppState): void {
    ...
    const rows: [string, string][] = [
      ...
      ["合同版本", task.contract_schema_version ?? "未知"],
      ["评审要求", label(REVIEW_REQUIREMENT_LABEL, task.review_requirement ?? null)],
      ["评审状态", label(REVIEW_STATE_LABEL, task.review_state ?? null)],
      ["兼容评审投影", label(REVIEW_LABEL, task.review_status)],
