...
- 当前工作树 Skill 包版本是 `0.8.2`（未发布开发线），Workflow Contract schema 继续是 `adf/v0.7.0`。
...
- 3 是自主 repair loop 上限，不是 AI 永久禁修。达到 `Stop` 后，用户可明确授权默认一次的有界 `EscalatedRepair`；失败回到 `Stop`，不得自动连跑。
...
- 当前工作树版本：`0.8.2`，尚未创建 tag / Release。
- 当前正式发布版本：`0.8.0`。
