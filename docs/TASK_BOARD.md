...
- `ADF-V010-RUNTIME-CONSOLE-BE`：`GPT Pro Needs Fix 0/1/0/0 / final implementation 5970ff5 / final Codex isolated pre-review Passed 0/0/0/0 / GPT Pro re-review Pending / Controlled / UA3 Pending / Pushed 798ad2d / Draft PR #16 / Unmerged`。`done` 已改为 end-only，Store/Schema/Builder/CLI 四层 fail-closed、空终态字段及真实磁盘无效会话可读原因码回归通过；等待 GPT Pro 对最终冻结 head 关闭 finding，当前不得进入 UA。
- `ADF-V010-PROJECT-CONSOLE-FE`：`GPT Pro prior #17 diff Passed 0/0/0/0 / updated stack Codex isolated pre-review Passed 0/0/0/0 / GPT Pro re-review Pending / Controlled / UA5 Pending user_only / final implementation 02fcce6 / branch pushed / Draft PR #17 / Unmerged`。普通 merge 已吸收 #16 最终 head；fresh backend `210/210`、Skill `121/121`、Vitest/Playwright 各 `109/109` 通过，full integration `52/53` 的唯一失败为 frozen artifact guard 且 `baseline_preserved=true`。旧 GPT Passed 不作为新 stack 外部收据，最终远端完整 SHA 以复审提示词为准。
