...
> - 当前阶段：`ADF-V010-MASTER In Progress / 前三阶段已交付 Draft PR / PROJECT-CONSOLE-FE In Progress / 后续两阶段 Draft`
...
- `ADF-V010-PROJECT-CONSOLE-FE`：`Candidate Ready / Controlled / Review Passed Round 3 0/0/0/0 / UA5 Pending user_only / Committed da82235 + receipt cd728eb / Pushed / Draft PR #17 / Unmerged`。base=`RUNTIME-CONSOLE-BE delivery head 95bc31d`；默认 Console、network/legacy 回退、revision/ETag/stale 保护、慢轮询与可访问性已通过全量自动化、真实浏览器 Design QA 和两轮 AutoRepair 后独立复审；session=`019fe1b4-e054-7500-b22a-ab233f72fc2c`。必须停在真实用户 UA，不得提前退役 Legacy。
