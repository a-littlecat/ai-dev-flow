...
> - 当前阶段：`ADF-V010-MASTER In Progress / CORE-SPLIT Review Passed，Committed / CAPABILITY-REVIEW Review Passed，Uncommitted / 后续四阶段 Draft`
...
- `ADF-V010-CAPABILITY-REVIEW`：`Review / Controlled / Review Passed / UA3 Pending / Uncommitted / Unmerged`。stacked base=`CORE-SPLIT delivery head 9a8642a`；能力驱动 Review、v0.10 Contract、Adapter 与最小 Dashboard 兼容已通过全量验证和 ER-1 独立复审；待阶段提交后进入 Runtime Console。
