...
- 非目标：不重跑或改写 V002/V003 评估；不批量迁移历史 TASK；不建设调度器、数据库、遥测、计费、模型 Adapter、Batch/Wave/Memory 平台；不执行 merge、push、tag、release、`Closed` 或本机 Skill 同步。
...
- 允许在专用分支形成本任务独立 commit；不授权 merge、push、tag、release、本机 Skill 同步或 `Closed`。
...
- 自动 merge、push、tag、release、外部同步或本机 Skill 安装
...
- UA 动作与结果：UA3 Pending；用户只需查看精简量、自动验证和独立 Review 证据，不需要逐行审代码。
- 发布决策：建议 v0.8 进入 UA3 / 后续发布决策；当前只认定“0.8.0 实现完成、尚未发布”，不创建 tag、push 或 Release。
...
- 当前为 `Review / Passed / UA3 Pending / Committed / Unmerged`。
- `VERSION=0.8.0` 只表示仓库内实现身份，不等于 tag、Published、Released、Merged、Accepted 或 Closed。
