...
- 外部 stacked finding 修复：原 P1/P2 已关闭；`bc7fd13` 双 Harness 共同 P3 `V010-CS-EXT6-001` 已修复，非 prototype 活跃文档不再复制固定 repair 预算/阈值/授权次数。fresh Skill `106/106`、workflow lint `errors=0 / violations=0 / warnings=1`、固定值扫描与 `git diff --check` 已通过；新 head 双 Harness 重审 Pending。backend `173/174`：`test_non_recursive_request_ignores_nested_file_changes` 在 #14 与无 Dashboard diff 的既有工作区均可复现，属于当前 Windows/Python 3.13 基线边界，本阶段未修改 `dashboard/**`。
