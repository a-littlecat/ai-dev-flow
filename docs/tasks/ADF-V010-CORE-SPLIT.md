...
- 外部 stacked finding 修复：原 P1/P2 已关闭；`385a4ec` 双 Harness 重审无 P0/P1/P2，Kimi 发现的两个 P3 已修复。定向 Policy Loader `12/12`、Skill 全量 `106/106`、workflow lint `errors=0 / violations=0 / warnings=1` 与 `git diff --check` 已通过；新 head 双 Harness 重审 Pending。backend `173/174`：`test_non_recursive_request_ignores_nested_file_changes` 在 #14 与无 Dashboard diff 的既有工作区均可复现，属于当前 Windows/Python 3.13 基线边界，本阶段未修改 `dashboard/**`。
