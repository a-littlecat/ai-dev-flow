...
- [ ] `CONTRACT-001` 已 `Accepted`，字段、枚举、不变量和诊断码已冻结。
- [ ] `git merge-base --is-ancestor <CONTRACT-001 Accepted commit> <当前 Base>` 成功。
- [ ] JSON Schema 与语义规范一致且可被标准 JSON parser 读取。
- [ ] 三个来源 commit 在当前 Git 历史中存在。
...
- [ ] 每个 fixture 有唯一 ID、真实输入路径和确定 expected result。
- [ ] 首批每条核心不变量至少有一个正例和一个反例。
- [ ] 首批每个 `E_* / V_* / W_*` diagnostic 至少有一个 fixture。
- [ ] `V_OVERLAY_WEAKENS_CORE` 标记为 `future_contract_007`，不冒充首批已实现。
- [ ] 覆盖重复 key、未知 key/value、ID 冲突、状态门禁、UA7、merge authority 和 feedback scope。
- [ ] legacy Review/merge 冲突明确预期 `E_LEGACY_CONFLICT`，且不产生确定 normalized 值。
- [ ] 未知 optional extension 保留并 warning；未知 required extension 按规范产生阻塞诊断。
- [ ] 三个真实样本均包含 legacy、Compact 和度量表。
- [ ] 每个 Compact 样本只有 8 个核心字段，不靠一排 `N/A / Not Applicable` 填充。
- [ ] 三个样本的 `compact_required_inputs` 均小于 `legacy_required_inputs`。
- [ ] 三个样本的 `compact_duplicate_state_occurrences` 均为 0，且严格小于对应的 `legacy_duplicate_state_occurrences`。
- [ ] fixture 不含私有路径、账号、密钥或项目外业务信息。
- [ ] 存在 `projects/valid-project/docs/tasks/` 形状的目录 fixture，用于验证 public project-root target，不把任意 fixture 目录当项目根。
- [ ] 独立 Review 无 P0/P1。
