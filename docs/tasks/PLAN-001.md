...
- 不接入或调用额外模型供应商；本计划阶段不执行当前模型真实任务对照。
...
- Tracked / Controlled 最多 3 轮 repair、4 轮 Review；更换 Reviewer 或当前执行模型版本不重置计数。
- 当前模型小样本真实任务对照总执行不超过 3 次，不接入额外模型 API、账号、CLI 或供应商。
...
2. `LEAN-002`：只制作默认关闭、可整体回退的最小原型，使用当前执行模型完成最多三次 Lite 小样本真实任务对照；不得全面迁移或默认启用。
...
- 本轮没有接入额外模型供应商，也没有执行当前模型真实任务对照、创建 `LEAN-*`、修改现行 Skill 或自批 Review。
- 用户要求消除模型来源歧义后，三份规划文档已统一为“当前执行模型真实任务对照”和“额外模型供应商”；该澄清不改变方案语义或既有验收结论。
...
- 问题：RFC 声明“先验证收益，再实施”，但候选顺序实际为 `LEAN-001` 历史回放、`LEAN-002` 完成 Skill 精简实现、`LEAN-003` 才做当前模型真实任务对照。
...
- Commit 状态：已提交
- 合并状态：已合并
- 合并目标与事实证据：main；Accepted 提交 `54ebc98`、术语澄清 `b7938ef` 与后续记录 `9c8ec36` 均为当前主线祖先
- 合并授权：User Authorized
- Commit 授权：用户于 2026-07-19 明确要求“提交”；本记录所在 commit 承载 PLAN-001 Accepted baseline，精确 hash 以 `git log` 为准。
- Accepted baseline：`54ebc98904beed7f98fe2acaecda005127ee5757`。
- 术语澄清 Commit 授权：用户于 2026-07-19 再次明确要求“提交”；本记录所在下一 commit 承载该澄清，精确 hash 以 `git log` 为准。
- 本轮术语澄清：已提交；只涉及 RFC、PLAN-001 和 TASK_BOARD 三份文档。
